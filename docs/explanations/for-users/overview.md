...
SeqQuest is a sequence analysis package which can be used via a web interface. It runs on Mac hardware.
...
* Methods for making, browsing and reducing an all-on-all Swissprot protein sequence comparison 
...
SeqQuest was developed for local use rather than for use by many people connecting over the internet. 

# Main Pipeline

* Prepares data files from the database to facilitate searching. SeqQuest will search with the longest protein sequences first and work down to the smallest. This step is about sorting and 'packing' the sequence files. The amino acid similarity table also needs to be prepared.
