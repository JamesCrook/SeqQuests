...
* Tooling for making and reducing an all-on-all Swissprot protein sequence comparison (2 days for 570,000 sequences all-on-all on Mac M4 Pro)
* LCARs (Star Trek TNG) inspired user interface for browsing the hits.

The code can be run via CLI/bash without using the web UI. However, the web UI makes exploring results easier, by dynamically making dotplots ~~and alignments~~. The server also supports a user interface for jobs. You can create, configure, start, stop and delete jobs, and monitor their progress.
...
Much of the code was created with AI assistance, for example python versions were translated from python to C++ using Jules, leaving behind a python interface to call the C++.