...
class LcarsUI {
  ...
  async doHelp() {
    ...
    try {
      ...
      */
      ...
      docs.forEach(doc => {
        html +=
          `<div class="block tan" style="cursor:pointer; padding: 10px; border-radius: 20px; text-align: center; color: black; font-weight: bold;" onclick="Lcars.showDoc('${doc.filename}')">${doc.name}</div>`;
      });
      html += '</div>';
  ...
  doJobQueue() {
    this.loadPartial('MainPanel', './panels/job_management_content.html',
      async () => {
        // Init job management
  ...
  asHtml(text) {
    ...
    if(text && !text.trim().startsWith('<')) {
      return '<div style="white-space: pre-wrap; font-family: inherit; padding: 10px;">' +
        text + '</div>';
    }
    return text;
  }
...
class HamburgerMenu {
  constructor(config) {
    this.config = config ?? [{
      ...
      label: 'Align',
      href: 'fast-align.html'
    }, {
      label: 'Matches',
      href: 'match_explorer.html'
    }, {
      label: 'Streaming Demo',
      href: 'stream_demo.html'
    }, {
      type: 'divider'
    }, {
      label: 'Tree',
      href: 'tree_browse.html'
    }, {
      type: 'divider'
    }, {
      label: 'Bias',
      href: 'bias_analysis.html'
    }, {
      label: 'Domains',
      href: 'domain-viewer.html'
    }, {
      label: 'Home',
      href: '/'
    }, {
      ...
      if(item.type === 'checkbox') {
        ...
        if(item.submenu) {
          ...
          el.onclick = (e) => {
            ...
            else if(typeof window[item.action] === 'function') window[
              item.action]();
          };
        }
      }
