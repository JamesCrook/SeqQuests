...
class ScrollMath {
   ...
   */
  static calculateTargetVisualTop(itemTop, itemHeight, viewportHeight,
    targetItemHeight) {
...
class SyncTarget {
  ...
  applySync(refBounds) {
    ...
    const translated = this.orch.nav.translateCursor(this.orch.activeCursor,
      this.level);
    const localIndex = translated[translated.length - 1];
    const item = this.el.querySelector(`[data-index="${localIndex}"]`) || this
    ...
    this.setInternalScrollPos(absoluteItemTop - targetVisualTop - this.el
      .offsetTop);
  }

  // --- Implementation Details ---
  getInternalScrollPos() {
...
class ManagedColumn extends SyncTarget {
  ...
  getCursorFromIndex(index) {
    return this.items[index]?.cursor || [0];
  }

  getBounds(cursor) {
    const translated = this.orch.nav.translateCursor(cursor, this.level);
    const item = this.items.find(it => JSON.stringify(it.cursor) === JSON
...
async function fetchSequenceDetailsFromLabServer(id1, id2) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/comparison/${id1}/${id2}`, {
        signal: controller.signal
      });
